...
const synchronous = (t, idx, opts) => {
  ...
  return `
${opts.map(o => sections[o].beforeFirst(fnName))}
${code};
${opts.map(o => sections[o].afterFirst(fnName))}
${code};

${opts.map(o => sections[o].beforeLast(fnName))}
var t${idx} = Date.now();
...
var cb${idx} = function (start${idx}) {
  return function (err, result) {
    ...
    ${opts.map(o => sections[o].afterLast(fnName))}
